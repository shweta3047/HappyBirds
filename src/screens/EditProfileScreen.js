...
const Profile = (props) => {
  ...
  const Gender=(props)=>{
    return (
      <RadioButton
      value={props} color={colors.secondary} 
  ...
  const submitHandler=()=>{
    props.updateUser(fullname,email,phone,age,checked,selectedConcerns, props.auth.user._id);
  }
           ...
           <AntDesign name="arrowleft" size={32} color="white" style={{position:"absolute",top:25,left:20}} 
            ...
            <Text style={styles.profileText}>Edit Profile</Text>
                <View style={styles.dpCover} >
                    <Image style={{width:90,height:90}} source={require('../../assets/userIcon.png')} />
                </View>
                ...
                <View style={{display:'flex', flexDirection:'row',alignItems:'center',padding:20,paddingTop:5,paddingBottom:5}}>
                  ...
                  <View>
                    ...
                    <View style={styles.problemsBox}>
                      {
                        concerns.map((prob)=>{
                        return <View key={prob.id} style={styles.problemsPart}>{Problem(prob)}</View>
                        })
                      }
                    </View>
                  </View>
                  <View style={{display:'flex',flexDirection:'row',justifyContent:'center'}}>
                    <Button mode="contained" color='#face4b' style={{width:'60%',borderRadius:20,marginTop:20}} 
