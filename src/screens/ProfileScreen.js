...
const ProfileScreen = (props) => {
    ...
    const [selectedConcerns, setSelectedConcerns] = useState(props.auth.profile.concerns);
    ...
    <ScrollView style={styles.container}>
      <View style={styles.background}>
          ...
          <View style={styles.dpCover}>
            <Image style={{width:100,height:100}} source={require('../../assets/userIcon.png')} />
          </View>
      </View>
      <View style={styles.info}>
          <Text style={styles.name}>{props.auth.profile.name}</Text>
          <View style={styles.infoBox}>
                <MaterialCommunityIcons name="gender-male-female" size={20} color={colors.tertiary} />
                <Text style={styles.otherInfo}>{props.auth.profile.gender}</Text>
          </View>
          <View style={styles.infoBox}>
                <MaterialCommunityIcons name="timer-sand-empty" size={20} color={colors.tertiary} />
                <Text style={styles.otherInfo}>{props.auth.profile.age} yrs. old</Text>
          </View>
          <View style={styles.infoBox}>
             ...
             <View style={{display:'flex',flexDirection:'row',flexWrap:'wrap'}}>
                 {
                     concerns.map(chip=>{
                         return (
                            <Chip key={chip.id} icon={selectedConcerns.includes(chip.id)?"check-circle-outline":"close-circle-outline"} 
                             disabled={selectedConcerns.includes(chip.id)? false: true} style={styles.chip}>
                                {chip.name}
                            </Chip>
                         )
                     })
